...
let casio = s("casio").struct("[x x x x]").degradeBy(0.4).sometimesBy(0.2, x=>x.fast(4)).att(slider(0.1,0, 0.1)).gain(slider(0.493,0,1)).room(.6)
  
let radio = s("sh5lostradiofx").gain(0.18)

let radio2 = s("sh5ringmodfx").gain("<0.08 0.04 0.02 0.01>")

let zim = s("spacedrum_oh").struct("[x x x x]").gain(0.8).fast(2).sometimesBy(slider(0.266, 0, 1) , x=>x.fast(2)).n(rand.range(0, 5)).o(2).att(slider(0.0561,0, 0.1)).gain(slider(0.659,0,1)).lpf(2500)

let hihat = s("hh hh hh hh").fast(2).sometimesBy(slider(0.518, 0, 1) , x=>x.fast(2)).n(rand.range(0, 5)).o(2).att(slider(0.0848,0, 0.1)).gain(slider(0.911,0,1))
...
let random_n = chooseCycles("A2","B2","C3","D3","E3","F3","~","~","~").note().s("xylophone_soft_pp").velocity(0.6).gain(4.2).room(.6).fast(8).degradeBy(0.3)

let continuo = ("<A1 C1>").s("supersaw").gain(1).lpf(300).room(1).dfb(1).slow(4)
  
let bass = note("A1 ~ A1 ~ D2 ~ E2 ~ A1 ~ [A1 E1] ~ A1 ~ A1 ~").s("gm_acoustic_bass").gain(0.6).slow(4).lpf(147)

let voice = note("<A1>").s("voicing").gain(0.5).room(.8)

let misc = s("yamaharm50_misc").gain(0.27).room(.9)

let song = arrange(
    [2, stack(radio)],
    [4, stack(radio, radio2)],
    [4, stack(radio, tambourine, bum, hihat)],
    [4, stack(radio, continuo, bum, hihat, snare, tambourine, bass)],
